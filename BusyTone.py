...
def checkBusyTone():
    for ru in range(NUM_RU):
        busy_tone_bits = np.zeros(NAS)  # NAS 길이의 초기화된 배열 생성
        ...
        same_busy_tone_count = np.sum(
            [sta.tx_status and sta.ru == ru and np.array_equal(sta.nas_binary, busy_tone_bits) for sta in
             stationList])
        if same_busy_tone_count >= 2:
            for sta in stationList:
                if sta.tx_status and sta.ru == ru and np.array_equal(sta.nas_binary, busy_tone_bits):
                    sta.status = False
